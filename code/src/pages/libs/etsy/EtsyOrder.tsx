...
export default function ({ Identifier, storeData }: any) {
  ...
  const [currentQueryString, setCurrentQueryString] = useState<any>(
  ...
  function handleSaveOrder(orderSignle = null, index = 0, orderState = orders) {
    ...
    return fetch(
      `https://sellercentral-api.inspireuplift.com/api/v1/seller/orders/${get(
        order,
        "id",
        ""
      )}?is_seller=true`,
        ...
        return {
          ...
          dataRequest: {
            ...
            shipping_info: {
              ...
              address_1: get(
                orderData,
                ["contact_info", "shipping_address", "address1"],
                ""
              ),
              address_2: get(
                orderData,
                ["contact_info", "shipping_address", "address2"],
                ""
              ),
              company: "",
              city: get(
                orderData,
                ["contact_info", "shipping_address", "city"],
                ""
              ),
              state: get(
                orderData,
                ["contact_info", "shipping_address", "province"],
                ""
              ),
              postcode: get(
                orderData,
                ["contact_info", "shipping_address", "zip"],
                ""
              ),
              country: get(
                orderData,
                ["contact_info", "shipping_address", "country_code"],
                ""
              ),
              email: get(orderData, ["contact_info", "email"], ""),
              phone: get(
                orderData,
                ["contact_info", "shipping_address", "phone"],
                ""
              ),
            },
      ...
      .then(async (data) => {
        return await Promise.all(
          map(
            unionBy(get(data, ["orderData", "line_items"], []), "product_id"),
            async (item: any) => {
          ...
          .then(async (items) => {
            if (
              ...
              if (isDetail()) {
                return await Promise.all(
                  map(
                    unionBy(get(data, ["orderData", "line_items"], []), "sku"),
                    async (item: any) => {
                      const attrTemp = $(
                        head(
                          $x(
                            `//span[text()="${item?.sku}"]/parent::p/parent::div/following-sibling::div[1]//p[1]/div`
                          )
                        ) as any
                      )?.text();
        ...
        return new Promise(async (resolve, reject) => {
          const getTransactionRequest = async (page = 1) => {
            return fetch(
              ...
              .then((response) => response.json())
              .then((data) => {
                const transactions = get(data, [
                  "data",
                  "transactions",
                  "data",
                ]);
                const lastPage = get(data, [
                  "data",
                  "transactions",
                  "last_page",
                ]);
      ...
      .then(async (data) => {
        ...
        const dataRequest = {
          ...
          items: map(get(data, ["dataRequest", "items"], []), (li) => {
            const itemData = find(
              items,
              (item) => get(item, "product_id") === get(li, "product_id", "")
            );
  ...
  function handleFillTracking(orderSignle = null) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      await fillTextInput(
        ...
        get(orderSyns, "tracking_number", "")
      );
      await clickXButton(
        `//label[text()="Shipping Carriers"]/following-sibling::div/div`
      );
      await sleep(1000);
      await fillTextInput(
        head(
          $x(
            `//label[text()="Shipping Carriers"]/following-sibling::div/div//input`
          )
        ),
  ...
  return (
    <>
                    ...
                    <tr key={get(orderSyns, "_id", "") + index + "tracking"}>
                      <td colSpan={3}>
                        <p className="text-right mb-1">
                          Tracking Number:{" "}
                          <strong>{orderSyns?.tracking_number}</strong>
                        </p>
                        <p className="text-right mb-1">
                          Tracking Carrier:{" "}
                          <strong>{orderSyns?.tracking_company}</strong>
                        </p>
                      </td>
                    </tr>
                  ) : (
      ...
      <BottomBar>
        ...
        >
          <RefreshCw size={14} />{" "}
          <span style={{ marginLeft: "3px" }}>Reload data</span>
        </Button>
        {isTracking() ? (
          <Button
            size="xs"
          ...
          >
            <Save size={14} />{" "}
            <span style={{ marginLeft: "3px" }}>Fill tracking</span>
          </Button>
        ) : (
          <Button
            size="xs"
            ...
            disabled={
              !currentDocker?.domain || !find(orders, (order) => order.checked)
            }
          >
            <Save size={14} />{" "}
            <span style={{ marginLeft: "3px" }}>Save orders</span>
          </Button>
        )}
      </BottomBar>
    </>
